...
Promise.all([
  ...
  var domainGdp = [200000.0,-200000.0]
  var domainLife = [1000.0,-1000.0]

  var gdpScale = d3.scaleLinear().range(['#dddddd','#96411B']);
  var lifeScale = d3.scaleLinear().range(['#dddddd','#aa66aa']);

  var gdpNumberScale = d3.scaleLinear()
  var lifeNumberScale = d3.scaleLinear()
  ...
  function loadAllData(error, gdp, life, rawtotal, imfrac, im, em, countriesData) {
    years.forEach( function(y) 
    {
        gdp.forEach( function(d) {
          allData['gdp'][y][d.id] = d[y]
          if(parseFloat(d[y]) > domainGdp[1]) domainGdp[1] = parseFloat(d[y])
          if(parseFloat(d[y]) < domainGdp[0]) domainGdp[0] = parseFloat(d[y])
        })
        life.forEach( function(d) {
          allData['life'][y][d.id] = d[y]
          if(parseFloat(d[y]) > domainLife[1]) domainLife[1] = parseFloat(d[y])
          if(parseFloat(d[y]) < domainLife[0]) domainLife[0] = parseFloat(d[y])
        })
        rawtotal.forEach( function(d) {total_migrants[y][d.id] = d[y]})
        imfrac.forEach( function(d) {im_fraction[y][d.id] = d[y]})
    });
    gdpScale.domain(domainGdp)
    lifeScale.domain(domainLife)
    gdpNumberScale.domain(domainGdp)
    lifeNumberScale.domain(domainLife)
    console.log(domainGdp)
    im_arcs = im; 
    em_arcs = em;
  ...
  d3.select('#reset-data-btn').on('click',clearArcs)
  d3.select('#reset-map-btn').on('click',reset)
  ...
  gdpNumberScale.range([0,colorBar.node().getBoundingClientRect().width-40]);

  lifeNumberScale.range([0,colorBar.node().getBoundingClientRect().width-40]);
      ...
      (d) => {
        ...
        switch(current_factor) {
          case "life":
            ...
            factorUnit = "Years"
            factorVal = parseFloat(factorVal).toFixed(2)
            break
          case "gdp":
            factorLabel = "GDP per Capita: "
            factorUnit = "USD"
            factorVal = parseFloat(factorVal).toFixed(1)
            break
        }
        return `
          <h4> 
  ...
  function drawArcs(country) {
    ...
    if(autoToggle && current_pos[0] != 0 && current_pos != 0) svg.transition().duration(1500).call(zoom.transform, d3.zoomIdentity.translate(center[0] - current_pos[0],center[1] - current_pos[1]));
